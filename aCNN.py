...
def grid(half_grid_size ):
    ...
    u_local = torch.arange(-half_grid_size, half_grid_size+1).cuda()
    v_local = u_local
    patchU, patchV = torch.meshgrid(u_local, v_local)
    dir_u_index = torch.reshape(patchV, (-1,)).cuda()
    dir_v_index = torch.reshape(patchU, (-1,)).cuda()
    return dir_u_index, dir_v_index

def Neighbors2D(depth, conv_filter = 3, dilation =1):
    ...
    coord_U = torch.arange(half_grid_size + dir_u_index[0], half_grid_size + feature_map_width + dir_u_index[0]).cuda()
    test_UU, test_VV = torch.meshgrid(coord_U, coord_V)
    torch_UU = test_UU.T.cuda()
    torch_VV = test_VV.T.cuda()
    for i in range(conv_filter) : 
        new[:, i::3,:,:,0] = torch_UU + i
        new[:, i*3:(i +1)*3,:,:,1] = torch_VV + i
        
    new[..., 2] = depth[:, new[...,1].long(), new[...,0].long()][:,0,...].cuda()
    new = new.permute(4,2,3,1,0).cuda()
    
    U_disp = torch.arange(half_grid_size, half_grid_size + feature_map_width).cuda()
    V_disp = torch.arange(half_grid_size, half_grid_size + feature_map_height).cuda()
    mat_coord_U_disp, mat_coord_V_disp = torch.meshgrid(U_disp, V_disp)
    mat_coord_U_disp = mat_coord_U_disp.T.cuda()
    mat_coord_V_disp = mat_coord_V_disp.T.cuda()
    centerFilterPosit = torch.zeros(2,  feature_map_height, feature_map_width).cuda()
    ...
    gridReg = torch.zeros(len(dir_u_disp), 2).cuda()
    gridReg[:,0] = dir_u_disp
    gridReg[:,1] = dir_v_disp
    allmap = torch.zeros(feature_map_height, feature_map_width, gridReg.shape[0], gridReg.shape[1]).cuda() + gridReg
...
def backProjection(Neighbors2d_posit ):
    ...
    u = Neighbors2d_posit[0,:].cuda()
    v = Neighbors2d_posit[1,:].cuda()
    z = Neighbors2d_posit[2,:].cuda()
    
    Neighbor2dPosit_2_3d = torch.zeros(Neighbors2d_posit.shape, dtype=torch.double).cuda()
...
def compute_plane(Neighbors2dPosit_2_3d ):
    Neighbors2dPosit_2_3d = Neighbors2dPosit_2_3d.cuda()
    ...
    A = torch.zeros((nbBatch, height, width, nbDePoints, 3), dtype=torch.double).cuda() # 2 colonnes
    A[:,:,:,:,0] = Neighbors2dPosit_2_3d[0,:].permute(3, 0, 1, 2)
    A[:,:,:,:,1] = Neighbors2dPosit_2_3d[1,:].permute(3, 0, 1, 2)
    A[:,:,:,:,2] = 1
    b = torch.zeros((nbBatch, height, width, nbDePoints, 1), dtype=torch.double).cuda() # 1 colonne
    b[:,:,:,:,0] = - Neighbors2dPosit_2_3d[2,:].permute(3, 0, 1, 2)



    A_transpose = A.permute(0,1,2,4,3) # A[:,:,:,i]  --- A_transpose[:,:,i,:]
    ata = torch.matmul(A_transpose, A).cuda()
    Atranspose_x_A_inv = torch.inverse(ata).cuda()
    pseudoInvA = torch.matmul(Atranspose_x_A_inv, A_transpose).cuda()
    abcd = torch.ones((nbBatch, pseudoInvA.shape[1], pseudoInvA.shape[2], pseudoInvA.shape[3]+1, 1), dtype=torch.double).cuda()
    abcd[:,:,:,:2,:] = torch.matmul(pseudoInvA, b)[:,:,:,:2,:].cuda()
    abcd[:,:,:,-1,:] = torch.matmul(pseudoInvA, b)[:,:,:,-1,:].cuda()
...
def uv3DPlane(NeighborsOn3dPlane, conv_filter ):
    ...
    u_candidates = NeighborsOn3dPlane[:,:,:,:,int((sizeConv-1)/2)::sizeConv].cuda()
    u_mean = torch.mean(torch.tensor(np.diff(u_candidates.cpu().data.numpy()), dtype = torch.double), axis = -1).cuda()
    u_norm = (u_mean / torch.norm(u_mean, dim= 0))
    u = torch.zeros(u_norm.shape[0],u_norm.shape[1],u_norm.shape[2],u_norm.shape[3], 1, dtype=torch.double).cuda()
    u[:,:,:,:,0] = u_norm
    
    v_candidates = NeighborsOn3dPlane[:,:,:,:,int((NeighborsOn3dPlane.shape[-1] - sizeConv)/2) : int((NeighborsOn3dPlane.shape[-1] + sizeConv)/2)].cuda()
    v_mean = torch.mean(torch.tensor(np.diff(v_candidates.cpu().data.numpy()), dtype = torch.double), axis = -1).cuda()
    v_norm = v_mean / torch.norm(v_mean, dim= 0)
    v = torch.zeros(u.shape, dtype=torch.double).cuda()
    v[:,:,:,:,0] = v_norm
...
def grid3D(NeighborsOn3dPlane, conv_filter, u_proj, v_proj, DepthCentralPixel, bol =False, dilation = 1):
    ...
    NeighborsOn3dPlane = NeighborsOn3dPlane.cuda()
    u_proj = u_proj.cuda()
    v_proj = v_proj.cuda()
    fw, fh, _, _ , _= cameraParams()

    point_3D_central = NeighborsOn3dPlane[:,:,:,:,int((conv_filter**2)/2)].cuda() # pt central est situe au milieu de la derniere dimension

    fv_indices, fu_indices = grid(int(conv_filter/2))

    gridReg = torch.zeros((len(fv_indices), 2), dtype=torch.double).cuda()
...
def GridProjection(grid_3D ):
    ...
    grid_2D = torch.zeros((grid_3D.shape[0], 2, grid_3D.shape[2], grid_3D.shape[3], grid_3D.shape[4]), dtype=torch.double).cuda()
...
def projRealPlane(Neighbors2dPosit_2_3d, abcd, conv_filter, dilation = 1):
    Neighbors2dPosit_2_3d = Neighbors2dPosit_2_3d.permute(0,4,1,2,3).cuda()
    nbPoints = Neighbors2dPosit_2_3d.shape[-1]
    positionCenter = int(nbPoints/2)
    DepthCentralPixel = Neighbors2dPosit_2_3d[2, :, :, :, positionCenter].mean(1).mean(1).cuda()
    u_3d , v_3d = orthoProjVect(abcd )
    proj,_ = grid3D(Neighbors2dPosit_2_3d, conv_filter, u_3d, v_3d, DepthCentralPixel, bol = True, dilation = dilation)
    return proj


def orthoProjVect(abcd ):
    zu = - abcd[..., 0, 0].cuda()
    u_3d = torch.zeros((3, abcd.shape[1], abcd.shape[2], abcd.shape[0]), dtype=torch.float).cuda()
    v_3d = torch.zeros((3, abcd.shape[1], abcd.shape[2], abcd.shape[0]), dtype=torch.float).cuda()
    z_3d = torch.zeros((3, abcd.shape[1], abcd.shape[2], abcd.shape[0]), dtype=torch.float).cuda()
    z_3d[2,: ]= 1
    ...
    abc = abcd[..., :3, 0].permute(3,1,2,0).cuda()
    norm = (torch.norm(abc,dim =0)**2).cuda()
    u_3d = z_3d - abc/norm
    u_3d = u_3d/ torch.norm(u_3d, dim = 0)
    v_3d = torch.cross(abc, u_3d).cuda()
    v_3d = v_3d/ torch.norm(v_3d, dim = 0)
   
    return u_3d.unsqueeze(1), v_3d.unsqueeze(1)


def computeOffset(depth, conv_filter = 3, dilation = 1):
    depth = depth.cuda()
    Neighbors2d_posit, dilated = Neighbors2D(depth, conv_filter, dilation = dilation)
    Neighbors2d_posit = Neighbors2d_posit.cuda()
    dilated = dilated.cuda()
    Neighbors2dPosit_2_3d = backProjection(Neighbors2d_posit ).cuda()
    abcd = compute_plane(Neighbors2dPosit_2_3d ).cuda()
    grid_3D_Reg = projRealPlane(Neighbors2dPosit_2_3d, abcd, conv_filter, dilation = dilation).cuda()

    grid_2D = GridProjection(grid_3D_Reg ).cuda()
    ori = dilated.cuda()
    inv_ori = torch.zeros(ori.shape).cuda()
    inv_ori[..., 0] = ori[..., 1]
    inv_ori[..., 1] = ori[..., 0]
    adapted = grid_2D.permute(2,3,4,0,1).cuda()
    inv_adapted = torch.zeros(adapted.shape).cuda()
    
    for i in range(conv_filter):
        inv_adapted[:,:,:, i*conv_filter: (i+1)*conv_filter] = adapted[:,:,:, i::conv_filter]
    
    diff = inv_adapted - inv_ori
    offset = diff.reshape(diff.shape[0], diff.shape[1], diff.shape[2], -1).cuda()
    offset  = offset.permute(0,3,1,2).cuda()
    return offset.float()
